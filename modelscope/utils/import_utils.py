...
def is_espnet_available(pkg_name):
    return importlib.util.find_spec("espnet2") is not None and importlib.util.find_spec(
        "espnet"
    )
...
REQUIREMENTS_MAAPING = OrderedDict(
    [
        ...
        (
            "espnet2",
            (is_espnet_available, GENERAL_IMPORT_ERROR.replace("REQ", "espnet")),
        ),
        (
            "espnet",
            (is_espnet_available, GENERAL_IMPORT_ERROR.replace("REQ", "espnet")),
        ...
        (
            "megatron_util",
            (is_package_available("megatron_util"), MEGATRON_UTIL_IMPORT_ERROR),
        ),
        (
            "text2sql_lgesql",
            (is_package_available("text2sql_lgesql"), TEXT2SQL_LGESQL_IMPORT_ERROR),
        ),
    ...
    def _try_to_import(self):
        for sub_module in self._class_to_module.keys():
            try:
                ...
                logger.warning(f"pre load module {sub_module} error, please check {e}")
